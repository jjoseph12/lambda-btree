...
def generate_synthetic_data(points, experiment_name):
    # Generate time steps directly based on the number of points
    time_step = np.arange(points)  # Use range from 0 to (points - 1)
    unique_entropy = np.cumsum(np.random.rand(points))  # Simulate cumulative unique entropy
...
p1 = figure(
    ...
    width=800,
    height=400,
    tools="tap,pan,box_zoom,wheel_zoom,reset"  # Added pan, box_zoom, wheel_zoom, and reset tools
)
...
p2 = figure(
    ...
    width=800,
    height=400,
    tools="pan,box_zoom,wheel_zoom,reset"  # Added the same tools
)
...
curdoc().title = "Dynamic Downsampling with Zoom"
